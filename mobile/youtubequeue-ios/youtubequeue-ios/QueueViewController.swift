...
class QueueViewController: CenterViewController, GCKDeviceScannerListener, GCKDeviceManagerDelegate,
    ...
    @IBAction func tappedAddButton() {
        ...
        let addURLAction = UIAlertAction(title: "add", style: .Default) {
            (_) in
            ...
            dispatch_after(delayTime, dispatch_get_main_queue(), {
                () -> Void in
                self.networkManager .addYoutubeLink(urlTextField.text!, roomId: self.roomId!, completionHandler: {
                    (success) -> Void in
                    self .refreshListData({
                        (success) -> Void in
    ...
    func videoFinishedPlaying() {
        ...
        if let video = currentVideo {
            networkManager.remove(video.id, roomId: roomId!, completionHandler: {
                (success) -> Void in
                self .refreshListData({
                    (success) -> Void in
                    self .playNextVideo()
    ...
    @IBAction func addRandomVideos(sender: AnyObject) {
        ...
        self.networkManager .addYoutubeLink("https://www.youtube.com/watch?v=UUlLGnTAHeM", roomId: roomId!, completionHandler: {
            (success) -> Void in
            ...
            dispatch_after(delayTime, dispatch_get_main_queue(), {
                () -> Void in
                self.networkManager .addYoutubeLink("https://www.youtube.com/watch?v=3LUw41dk4JY", roomId: self.roomId!, completionHandler: {
                    (success) -> Void in
                    self.refreshListData({
                        (success) -> Void in
                    })
...
extension QueueViewController {
    func downVoteButtonPressed(video: Video) {
        self.networkManager .downvote(video.id, roomId: roomId!) {
            (success) -> Void in
            self .refreshListData({
                (success) -> Void in
            })
    ...
    func upVoteButtonPressed(video: Video) {
        self.networkManager .upvote(video.id, roomId: roomId!) {
            (success) -> Void in
            self .refreshListData({
                (success) -> Void in
            })
